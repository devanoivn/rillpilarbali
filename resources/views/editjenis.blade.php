...
<form action="{{ route('jenis.update', $jenis->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
        <label for="username" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> Category Name</label>
        <div class="mt-1 sm:mt-0 sm:col-span-2">
            <div class="max-w-lg flex rounded-md shadow-sm">

                <input type="text" name="jenis" id="username" autocomplete="username"
                    class="form-control flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-none rounded-r-md sm:text-sm border-gray-300" value="{{ $jenis->jenis }}">
                @if($errors->has('jenis'))
    ...
    <div class="pt-5">
        <a href="/admin/listjenis"><div class="">
            <button type="button"
