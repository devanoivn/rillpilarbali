...
<form action="{{ route('kayus.update', $kayu->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="_method" value="PATCH">
    <div class="mb-5">
        <h3 class="text-lg leading-6 font-medium text-gray-900">Add Wood Material</h3>
    </div>
    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
        <label for="username" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> Material Wood Name
        </label>
        <div class="mt-1 sm:mt-0 sm:col-span-2">
            <div class="max-w-lg flex rounded-md shadow-sm">

                <input type="text" name="kayu" id="username" autocomplete="username"
                    class="flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-none rounded-r-md sm:text-sm border-gray-300"  value="{{ $kayu->kayu }}">
                @if($errors->has('kayu'))
                <p class="text-danger">{{ $errors->first('kayu') }}</p>
                @endif
            </div>
        </div>
    </div>
